...
def plot_voronoi_diagram(points, vor=None, show_voronoi=True):
    fig, ax = plt.subplots(figsize=(10, 10))

    if points:
        px, py = zip(*points)

        if show_voronoi and vor is not None:
            # rysuj krawędzie
            for ridge in vor.ridge_vertices:
                ...
                ax.plot([v0[0], v1[0]], [v0[1], v1[1]],
                        color="forestgreen", linewidth=1.5, zorder=1)

            # wierzchołki
            ax.scatter(vor.vertices[:, 0], vor.vertices[:, 1],
                       c="red", s=20, label="Wierzchołki Voronoi", zorder=2)

        ax.scatter(px, py, c="blue", s=100, label="Punkty generujące", zorder=3)
...
def voronoi_ridge_segments(vor, radius=1000):
    """
    Zamienia diagram Voronoi na listę odcinków (również dla promieni nieskończonych)
    Zwraca: [(p0, p1), ...]
    """
    ...
    for (p1, p2), ridge in zip(vor.ridge_points, vor.ridge_vertices):
        ...
        if -1 not in ridge:
            v0, v1 = vor.vertices[ridge]
            new_segments.append((v0, v1))
            continue

        # promień
        i = ridge[0] if ridge[0] != -1 else ridge[1]
        v = vor.vertices[i]
        ...
        new_segments.append((v, far_point))
...
def create_folium_map(geo_points, dcel=None, show_voronoi=True, center_lat=52.0, center_lon=19.0):
    ...
    if show_voronoi and dcel and geo_points:
        ...
        segments = voronoi_ridge_segments(vor, radius=5000)

        for v0, v1 in segments:
            lat1, lon1 = xy_to_lat_lon(v0[0], v0[1], center_lat, center_lon)
            lat2, lon2 = xy_to_lat_lon(v1[0], v1[1], center_lat, center_lon)
        ...
        for v in vor.vertices:
            lat, lon = xy_to_lat_lon(v[0], v[1], center_lat, center_lon)
            folium.CircleMarker(
                location=[lat, lon],
                radius=3,
                color='red',
...
def generate_voronoi(points):
    """Generuje diagram Voronoi dla podanych punktów (SciPy)"""
    if len(points) < 3:
        return None
...
with col_left:
    ...
    if st.session_state.map_mode == 'matplotlib':
        ...
        if st.session_state.geo_points:
            ...
            # Wyświetl mapę
            map_data = st_folium(folium_map, width=1200, height=1200)
            ...
            map_data = st_folium(empty_map, width=1200, height=1200)
...
with col_right:
    ...
    if current_points:
        ...
        if st.session_state.diagram_generated and st.session_state.dcel:
            vor = st.session_state.dcel
            edges = sum(1 for r in vor.ridge_vertices if -1 not in r)

            st.metric("Wierzchołki Voronoi", len(vor.vertices))
            st.metric("Krawędzie", edges)
            st.metric("Regiony", len(vor.regions))
